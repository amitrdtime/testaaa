...
const PlannerBoardDriverViewBodyFormatted = (props) => {
  ...
  const previousDayChange = () => {
    ...
    if (startdate) {
      ...
      var currentdate = new Date(Date.now() + -1 * 24 * 60 * 60 * 1000);
    }
    getCompleteDate(currentdate);
    var filterData = {};
    filterData["date"] = new Date(currentdate);
...
export default PlannerBoardDriverViewBodyFormatted;
